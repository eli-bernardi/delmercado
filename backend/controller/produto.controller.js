...
const cadastrar = async (req,res)=>{
    ...
    if(!valores.nome || !valores.descricao || !valores.categoria || 
        !valores.preco || !valores.percentualDesconto || !valores.quantidade || 
...
// Carga em lote de produtos a partir da API DummyJSON
const bulkLoad = async (req,res)=>{
    try{
        ...
        console.error('Erro no carregamento em lote de produtos',err)
        res.status(500).json({message: 'Erro no carregamento em lote de produtos'})
    }
}
